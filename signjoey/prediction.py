...
def validate_on_data(
    ...
    txtcntcls_loss_function: torch.nn.Module,
    recognition_loss_weight: int,
    txtcntcls_loss_weight: int,
    do_translation: bool,
    ...
    with torch.no_grad():
        ...
        for valid_batch in iter(valid_iter):
            ...
            batch_recognition_loss, batch_translation_loss, batch_txtcntcls_loss = model.get_loss_for_batch(
                batch=batch,
                recognition_loss_function=recognition_loss_function
                if do_recognition
                else None,
                txtcntcls_loss_function=txtcntcls_loss_function
                if do_recognition
                else None,
                ...
                txtcntcls_loss_weight=txtcntcls_loss_weight
                if do_recognition
                else None,
            ...
            if do_recognition:
                ...
                total_txtcntcls_loss += batch_txtcntcls_loss
    ...
    if do_recognition:
        ...
        results["valid_txtcntcls_loss"] = total_txtcntcls_loss
...
def test(
    ...
    if do_recognition:
        ...
        txtcntcls_loss_function = XentLoss(
            pad_index=txt_vocab.stoi[PAD_TOKEN], smoothing=0.0
        )
        if use_cuda:
            txtcntcls_loss_function.cuda()
    if do_translation:
        ...
        for tbw in translation_beam_sizes:
            ...
            for ta in translation_beam_alphas:
                dev_translation_results[tbw][ta] = validate_on_data(
                    ...
                    txtcntcls_loss_weight=1 if do_recognition else None,
    ...
    test_best_result = validate_on_data(
        ...
        recognition_loss_weight=1 if do_recognition else None,
        txtcntcls_loss_function=txtcntcls_loss_function if do_recognition else None,
        txtcntcls_loss_weight=1 if do_recognition else None,
