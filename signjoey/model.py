...
class SignModel(nn.Module):
    ...
    def __init__(
        ...
        self.txtcnt_layer = txtcnt_layer
    ...
    def forward(
        ...
        """
        encoder_output, encoder_hidden, cls_token = self.encode(     
            sgn=sgn, sgn_mask=sgn_mask, sgn_length=sgn_lengths
        )   # 需要保证 cls_token 的维度是 N x 1 x C
        ...
        if self.do_recognition:
            ...
            txtcnt_probabilities = self.txtcnt_layer(cls_token)  
            # txtcnt_probabilities = txtcnt_scores.log_softmax(2)
            # txtcnt_probabilities = txtcnt_probabilities.permute(1, 0, 2)
            

        else:
            gloss_probabilities = None
            txtcnt_probabilities = None
        ...
        return decoder_outputs, gloss_probabilities, txtcnt_probabilities
    ...
    def get_loss_for_batch(
        ...
        # Do a forward pass
        decoder_outputs, gloss_probabilities, txtcnt_probabilities = self.forward(
            sgn=batch.sgn,
        ...
        if self.do_recognition:
            ...
            assert txtcnt_probabilities is not None
            ...
            txtcnt_probabilities = F.log_softmax(txtcnt_probabilities, dim = -1)
            # txtcnt_prob_2 = F.log_softmax(cls_token_2, dim = -1)
            # txtcnt_probabilities = torch.add(txtcnt_probabilities, txtcnt_prob_2).cuda()  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
            txtcntcls_trg = batch.gls_lengths
            txtcntcls_trg = txtcntcls_trg.unsqueeze(-1).cuda()

            txtcntcls_loss = (
                    txtcntcls_loss_function(txtcnt_probabilities, txtcntcls_trg)
                    * txtcntcls_loss_weight
            )
        else:
        ...
        return recognition_loss, translation_loss, txtcntcls_loss
...
def build_model(
    ...
    if do_recognition:
        gloss_output_layer = nn.Linear(encoder.output_size, len(gls_vocab))  # output_size 应该是512， 但哪里来的？
        txtcnt_layer = nn.Linear(encoder.output_size, 30) 
