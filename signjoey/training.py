...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        if self.do_recognition:
            ...
            self._get_txtcntcls_params(train_config=train_config)
        ...
        if self.use_cuda:
            ...
            if self.do_recognition:
                ...
                self.txtcntcls_loss_function.cuda()
    ...
    def _get_txtcntcls_params(self, train_config) -> None:
        self.label_smoothing = train_config.get("label_smoothing", 0.0)
        self.txtcntcls_loss_function = XentLoss(
            pad_index=self.txt_pad_index, smoothing=self.label_smoothing
        )
        self.txtcntcls_normalization_mode = train_config.get(
            "txtcntcls_normalization", "batch"
        )
        if self.txtcntcls_normalization_mode not in ["batch", "tokens"]:
            raise ValueError(
                "Invalid normalization {}.".format(self.txtcntcls_normalization_mode)
            )
        self.txtcntcls_loss_weight = train_config.get("txtcntcls_loss_weight", 1.0)
    ...
    def train_and_validate(self, train_data: Dataset, valid_data: Dataset) -> None:
        ...
        for epoch_no in range(self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                recognition_loss, translation_loss, txtcntcls_loss = self._train_batch(
                    batch, update=update
                )
                ...
                if self.do_recognition:
                    ...
                    self.tb_writer.add_scalar(
                        "train/train_txtcntcls_loss", txtcntcls_loss, self.steps
                    )
                    epoch_txtcntcls_loss += txtcntcls_loss.detach().cpu().numpy()
                ...
                if self.steps % self.logging_freq == 0 and update:
                    ...
                    if self.do_recognition:
                        ...
                        log_out += "Batch Txtcntcls Loss: {:10.6f} => ".format(
                            txtcntcls_loss
                        )
                    if self.do_translation:
                        elapsed_txt_tokens = (
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    val_res = validate_on_data(
                        ...
                        txtcntcls_loss_function=self.txtcntcls_loss_function
                        if self.do_recognition
                        else None,
                        recognition_loss_weight=self.recognition_loss_weight
                        if self.do_recognition
                        else None,
                        txtcntcls_loss_weight=self.txtcntcls_loss_weight
                        if self.do_recognition
                        else None,
                    ...
                    if self.do_recognition:
                        ...
                        self.tb_writer.add_scalar(
                            "valid/valid_txtcntcls_loss",
                            val_res["valid_txtcntcls_loss"],
                            self.steps,
                        )
                        self.tb_writer.add_scalar(
                    ...
                    self._add_report(
                        ...
                        valid_txtcntcls_loss=val_res["valid_txtcntcls_loss"]
                        if self.do_recognition
                        else None,
    ...
    def _train_batch(self, batch: Batch, update: bool = True) -> (Tensor, Tensor):
        ...
        recognition_loss, translation_loss, txtcntcls_loss = self.model.get_loss_for_batch(
            batch=batch,
            recognition_loss_function=self.recognition_loss_function
            if self.do_recognition
            else None,
        ...
        if self.do_recognition:
            ...
            if self.txtcntcls_normalization_mode == "batch":
                txtcntcls_normalization_factor = batch.num_seqs
            elif self.txtcntcls_normalization_mode == "tokens":
                txtcntcls_normalization_factor = batch.num_txt_tokens
            else:
                raise NotImplementedError("Only normalize by 'batch' or 'tokens'")

                # division needed since loss.backward sums the gradients until updated
            normalized_txtcntcls_loss = txtcntcls_loss / (
                    txtcntcls_normalization_factor * self.batch_multiplier
            )   
        else:
            normalized_recognition_loss = 0
            normalized_txtcntcls_loss = 0

        total_loss = normalized_recognition_loss + normalized_translation_loss + normalized_txtcntcls_loss
        # compute gradients
        total_loss.backward()
        ...
        return normalized_recognition_loss, normalized_translation_loss, normalized_txtcntcls_loss

    def _add_report(
            self,
            valid_scores: Dict,
            valid_recognition_loss: float,
            valid_txtcntcls_loss: float,
        ...
        with open(self.valid_report_file, "a", encoding="utf-8") as opened_file:
            opened_file.write(
                ...
                "LR: {:.8f}\t{}\n".format(
                    ...
                    valid_txtcntcls_loss if self.do_recognition else -1,
